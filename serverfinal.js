...
// === Groq chat completion with query ===
async function getGroqChatCompletionForQuery(query) {
  console.log("🔹 Sending query to Groq API:", query); // Log the query being sent
  ...
  try {
    ...
    console.log("✅ Groq API response:", response); // Log the full response from Groq

    if (!response.choices || response.choices.length === 0) {
      console.error("❌ No choices in Groq response:", response);
    ...
    console.error("❌ Groq API error:", error); // Log detailed error
    return "Error processing query with Groq: " + error.message; // Return detailed error message
  }
}
